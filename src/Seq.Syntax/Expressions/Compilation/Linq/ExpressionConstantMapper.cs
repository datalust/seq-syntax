...
class ExpressionConstantMapper : ExpressionVisitor
{
    ...
    {
        if (node.Value is EvaluationResult result &&
            result.TryGetValue(out var payload) &&
            payload is JsonValue value &&
            _mapping.TryGetValue(Values.Underlying(value), out var substitute))
            return substitute;
