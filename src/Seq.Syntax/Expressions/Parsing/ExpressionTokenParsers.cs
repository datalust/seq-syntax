...
static class ExpressionTokenParsers
{
    ...
    static readonly TokenListParser<ExpressionToken, Expression> String =
        Token.EqualTo(ExpressionToken.String)
            ...
            .Select(s => (Expression)new ConstantExpression(JsonValue.Create(s)));
    ...
    static readonly TokenListParser<ExpressionToken, Expression> HexNumber =
        Token.EqualTo(ExpressionToken.HexNumber)
            ...
            .Select(u => (Expression)new ConstantExpression(JsonValue.Create((decimal)u)));
    ...
    static readonly TokenListParser<ExpressionToken, Expression> Number =
        Token.EqualTo(ExpressionToken.Number)
            ...
            .Select(d => (Expression)new ConstantExpression(JsonValue.Create(d)));
    ...
    static readonly TokenListParser<ExpressionToken, Expression> Literal =
        String
            .Or(Number)
            .Or(HexNumber)
            .Or(Token.EqualTo(ExpressionToken.True).Value((Expression)new ConstantExpression(JsonValue.Create(true))))
            .Or(Token.EqualTo(ExpressionToken.False).Value((Expression)new ConstantExpression(JsonValue.Create(false))))
            .Or(Token.EqualTo(ExpressionToken.Null).Value((Expression)new ConstantExpression(EvaluationResult.Null)))
            .Named("literal");
    ...
    public static readonly TokenListParser<ExpressionToken, Expression> Expr = input =>
    {
        // All structural recursion re-enters through `Expr` (via `Parse.Ref`), so guarding it here bounds
        // parser stack depth for nested parens/arrays/objects/conditionals.
