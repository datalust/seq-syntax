...
class TemplateTokenParsers
{
    ...
    public TemplateTokenParsers()
    {
        ...
        var alignment =
            Token.EqualTo(Comma).IgnoreThen(
                (from direction in Token.EqualTo(Minus).Value(AlignmentDirection.Left)
                    ...
                    // Clamp rather than allocate unbounded padding.
                    select new Alignment(direction, (int)Math.Min(width, Alignment.MaxWidth))).Named("alignment and width"));
        ...
        var blockBody = element.Many().Select(elements => elements.Length == 1 ?
            elements[0] :
            new TemplateBlock(elements));

        // Nested directives re-enter through `block` (via `Parse.Ref`); guard parser stack depth.
        block = input =>
        {
            RuntimeHelpers.EnsureSufficientExecutionStack();
            return blockBody(input);
        };
